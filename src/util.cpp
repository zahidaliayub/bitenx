...
const char * const BITCOIN_CONF_FILENAME = "bitenx.conf";
const char * const BITCOIN_PID_FILENAME = "bitenxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bitenx" is a composite category enabling all BitenX-related debug output
            if(ptrCategory->count(string("bitenx"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitenX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitenX
    // Mac: ~/Library/Application Support/BitenX
    // Unix: ~/.bitenx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitenX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitenX";
#else
    // Unix
    return pathRet / ".bitenx";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitenx.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bitenx.conf
