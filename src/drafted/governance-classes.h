 ...
 *
 *   // network
 *   CBitenXNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.bitenx.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBitenXNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: BitenXNetwork
// //   -- signature requirements : Key1(User)
// class CBitenXNetworkVariable : public CGovernanceObject
// {
// private:
