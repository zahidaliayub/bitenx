...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivatePayRoundsMin, int nPrivatePayRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nPrivatePayRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT5000IFMN : ONLY_DENOMINATED);
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstaPay))
                {
                    if (nCoinType == ONLY_NOT5000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 BTENX.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT5000IFMN) {
                        strFailReason = _("Unable to locate enough PrivatePay non-denominated funds for this transaction that are not equal 1000 BTENX.");
                    ...
                    if (fUseInstaPay) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstaPay doesn't support sending values that high yet. Transactions are currently limited to %1 BTENX."), sporkManager.GetSporkValue(SPORK_5_INSTAPAY_MAX_VALUE));
                        } else {
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-bitenx-address
                        CScript scriptChange;
